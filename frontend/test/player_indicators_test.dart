...
PlayerIndicators _indicators(int id, {bool missingWage = false}) =>
    PlayerIndicators(
      playerId: id,
...
void main() {
  test('error-based cost grades accept null percentile while Form requires it',
      () {
    ...
    expect(() => ApiPlayerIndicatorsResponse.fromJson(payload),
        throwsFormatException);
  });

  test('API requests current season by provider ID and preserves null wages',
      () async {
  ...
  test('API distinguishes missing current roster from request failure',
      () async {
    var status = 404;
    final repository = ApiPlayerIndicatorsRepository(
      client: MockClient((_) async => http.Response(
          '{"detail":"Player not in a current five-league squad"}', status)),
      apiBaseUri: Uri.parse('https://example.test/v1/'),
    ...
    status = 503;
    await expectLater(
        repository.loadCurrent(997), throwsA(isA<http.ClientException>()));
      ...
      testWidgets(
        ...
        await tester.pumpWidget(MaterialApp(
          ...
          home: Scaffold(
            ...
            child: PlayerBioStatsBlock(player: player, repository: repository),
          )),
        ));
        ...
        await tester.pumpAndSettle();
        expect(find.textContaining('Transfer fees are not included.'),
            findsOneWidget);
      });
    }
  }
  ...
  testWidgets('missing wage displays no invented grade or ring',
    ...
    await tester.pumpWidget(MaterialApp(
      ...
      player: player,
      repository: _Repository((id) async => _indicators(id, missingWage: true)),
    ))));
    await tester.pumpAndSettle();
    expect(find.text('Fair'), findsOneWidget);
    ...
    expect(find.descendant(of: cost, matching: find.byType(CustomPaint)),
        findsNothing);
  });
  ...
  testWidgets('old response cannot overwrite a newly selected player',
    ...
    final repository =
        _Repository((id) => id == 997 ? first.future : second.future);
