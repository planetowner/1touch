        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () {
                  context.push('/team/${team.id}'); // Or use ID if your route expects it
                },
                child: Row(
                  children: [
              ...
              if (match != null)
                GestureDetector(
                  onTap: () {
                    ...
                    final status = helper.determineMatchStatus(matchDateTime);
                    context.push('/match/${match.id}?status=$status');
                  },
                  child: helper.MatchCard(
                    date: _formatDate(match.date),
                    venue: "Venue ID ${match.venueId}",
                    team1Name: match.homeTeam?.name ?? "Home",
                    team1Logo: match.homeTeam?.imagePath ?? "",
                    team2Name: match.awayTeam?.name ?? "Away",
                    team2Logo: match.awayTeam?.imagePath ?? "",
                  ),
                ),
...
class HardcodedCalendar extends StatefulWidget {
  const HardcodedCalendar({Key? key}) : super(key: key);
...
class _HardcodedCalendarState extends State<HardcodedCalendar> {
  DateTime _currentMonth = DateTime(DateTime.now().year, DateTime.now().month, 1); // Start with April 2025
              ...
              if (week < weekCount - 1)
                LayoutBuilder(
                  builder: (context, constraints) {
                    final cellW  = constraints.maxWidth / 7.0;
                    final left  = 12.0;
                    final right  = (week == weekCount - 2) ? (6 - lastDayIndex) * cellW : 12.0;
                    return Container(
