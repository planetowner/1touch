...
def _result_for_team(
    home_id: int, away_id: int, hs: Optional[int], as_: Optional[int], team_id: int
) -> Optional[str]:
...
def _last5_for_team(
    fixtures: List[Tuple[int, int, Optional[int], Optional[int], str]], team_id: int
) -> List[str]:
    """
...
def _normalize_knockout_round(name: str) -> str:
    n = name.lower()
    if "round of 16" in n or "1/8" in n or "r16" in n:
        return "Round of 16"
    if "quarter" in n or "qf" in n:
        return "Quarter-finals"
    if "semi" in n or "sf" in n:
        return "Semi-finals"
    if "final" in n and "semi" not in n and "quarter" not in n:
        return "Final"
    if "knockout round" in n:
...
def _home_away_from_fx(fx: dict) -> Tuple[Optional[int], Optional[int]]:
    ...
    if home is None or away is None:
        for s in fx.get("scores") or []:
            ...
            if side == "home" and isinstance(pid, int):
                home = home or pid
            elif side == "away" and isinstance(pid, int):
                away = away or pid
    return home, away
...
def build_league_standings_for_season(league_id: int, season_id: int):
    ...
    AND season_id=%s
    AND status='past'
    AND COALESCE(round_name, '') REGEXP '^[0-9]+$'
    ORDER BY starting_at DESC
    """
    rows = fetch_all(sql, (league_id, season_id))
    # last5 계산을 위해 최신순 그대로 보존
    latest_by_team: Dict[
        int, List[Tuple[int, int, Optional[int], Optional[int], str]]
    ] = defaultdict(list)
    # 누계 집계
...
def build_euro_phase_standings_for_season_db(league_id: int, season_id: int):
    ...
    groups: Dict[str, List[Tuple[int, int, Optional[int], Optional[int], str]]] = (
        defaultdict(list)
    )
...
def build_knockout_brackets_for_season(league_id: int, season_id: int):
    ...
    row = fetch_all(
        "SELECT name, starting_at FROM seasons WHERE season_id=%s", (season_id,)
    )
    start_year = None
    if row:
        n = row[0][0] or ""
    ...
    for (rname, t1, t2), games in grouped.items():
        ...
        games.sort(
            key=lambda x: (x[8], x[7], x[0])
        )  # (starting_at, leg_number, fixture_id)
        ...
        for fxid, h, a, hs, as_, p_h, p_a, leg, dt in games:
            ...
            if league_id in (2, 5, 2286) and (
                start_year is not None and start_year <= 2020
            ):
                if team1_home:
                    # team2가 원정
                    away2 += g2
        ...
        if agg1 != agg2:
            ...
            league_id in (2, 5, 2286)
            and (start_year is not None and start_year <= 2020)
            and (away1 != away2)
        ):
...
def compute_rank_delta_since_last_match(
    ...
    before = _compute_position_asof(
        league_id, season_id, cutoff=last_dt, include_cutoff=False
    )
    after = _compute_position_asof(
        league_id, season_id, cutoff=last_dt, include_cutoff=True
    )
...
def _compute_position_asof(
    league_id: int, season_id: int, cutoff: str, include_cutoff: bool
) -> Dict[int, int]:
    """
