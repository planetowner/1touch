...
| Opta 슈팅·패스·수비 | 종료 경기 탐지 후 첫 수집. 자료 없음은 킥오프 후 48시간까지 5분마다, 이후 24시간마다 재확인 | 종료 상세·명단 → 검증된 경기·선수 연결 → 한 번의 분석 화면 수집으로 슈팅과 분석 저장 |
...
2026-09-23 Opta 수정: 한 실행에서 최대 5경기, 그중 킥오프 후 48시간이 지난 경기는 최대 1경기를 처리한다. 최근 경기부터 처리하며 오래된 경기는 마지막 시도가 오래된 순서로 나눠 처리한다. 이는 경기 수 제한이며 실행 시간의 상한은 아니다. 공급자 응답 시간과 서버 CPU 제한에 따라 다음 시도는 예약 간격보다 늦어질 수 있다.

Opta 화면에 두 가지 미제공 문구가 모두 확인된 경우만 `unavailable`로 기록한다. 시간 초과·매핑 오류는 `failed`로 남기고 기존 5분 간격을 유지한다. 결과를 경기마다 바로 저장하므로 뒤 경기의 실패나 실행 중단이 앞 경기의 결과를 지우지 않는다. 시즌별 보고서도 서로 다른 폴더에 저장한다.

DB 경기 ID가 없는 예선은 `opta:<원본 경기 ID>` 키로 재확인 시각을 저장한다. 검증된 경기 ID 또는 양 팀 ID와 날짜가 유일하게 맞을 때만 DB 경기 기록에도 연결한다. 원본 대회·시즌·홈/원정·날짜·시각이 바뀌거나 DB 경기 결과가 정정되면 기존 대기를 풀고 확인한다. 원본 킥오프 시각이 없으면 48시간 경과를 단정하지 않고 5분 간격을 유지한다.

기존 원본 파일에서 미제공 기록을 복구하는 명령은 `python -m diagnostics.reconcile_opta_refresh --check`다. 기본은 조회이며 `--apply`만 `opta.json`을 변경한다. 적용 전 Opta 예약과 실행 중인 Opta 컨테이너를 멈춰야 하며, 원래 상태는 `opta.before-reconcile-*.json`으로 보관한다. DB 자료나 경기 ID 관계를 새로 쓰지 않는다.
