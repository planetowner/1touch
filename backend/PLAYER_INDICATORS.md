...
- 전체 비교 집단의 계산 결과를 프로세스 안에서 공유하며, 시간만으로 만료시키지 않아요.
- 요청마다 같은 읽기 전용 트랜잭션으로 원본 입력을 확인하고, 입력이 같으면 기존 계산 결과를 반환해요. 응답의 `as_of`도 실제 계산 기준 시각을 유지해요.
- 현재 시즌 5대 리그의 종료 경기·평점·출전 시간, 명단·포지션·급여·스쿼드 역할, 적용 보정값이 바뀌면 다음 조회에서 전체 결과를 다시 계산해요. 다른 팀의 경기와 기존 기록의 정정·삭제도 반영해요.
