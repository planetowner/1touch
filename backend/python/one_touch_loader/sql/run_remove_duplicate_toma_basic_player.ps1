...
$sqlPath = Join-Path $PSScriptRoot 'remove_duplicate_toma_basic_player.sql'
$timestamp = Get-Date -Format 'yyyyMMddTHHmmss'
$backupDirectory = Join-Path $backendRoot "logs\database-backups\$timestamp"
$backupPath = Join-Path $backupDirectory 'before_duplicate_toma_basic_removal.sql'
$backupErrorPath = Join-Path $backupDirectory 'before_duplicate_toma_basic_removal.stderr.log'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Duplicate Toma Basic removal failed with exit code $LASTEXITCODE"
    }

    Write-Host 'Duplicate Toma Basic removal completed.'
