...
def get_shotmap(fixture_id: int) -> dict:
    ...
    rows = fetch_all_dict("""
        SELECT src.source_url,m.collected_at,f.home_team_id,f.away_team_id,
    ...
    result.update(source_url=meta["source_url"],
                  ...
                  counts={side: sum(row['external_event_id'] is not None and row['team_id'] == meta[f'{side}_team_id']
                                    for row in rows) for side in ('home','away')})
    shots = []
    for row in rows:
        if row["external_event_id"] is None:
            continue
        for key in ("source_url", "collected_at", "home_team_id", "away_team_id"):
            row.pop(key)
